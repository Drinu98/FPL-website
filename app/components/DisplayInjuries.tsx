...
function Injuries(props: InjuriesProps) {
  ...
  for (const team in injuries) {
    const playersWithIAndDStatus = injuries[team].filter(
      (player: any) => player.status === "i" || player.status === "d"
    );
        ...
        <tbody>
          {playersForSelectedTeam?.map((player: any, index: any) => (
            <tr key={index} className="table-row">
              <td style={{ paddingRight: "10px", paddingLeft: "10px" }}>
                {player.status === "i" ? (
                  <Image
                    ...
                    width={22}
                    height={22}
                  /> /* Display red flag for status 'i' */
                ) : player.chanceOfPlaying &&
                  parseInt(player.chanceOfPlaying) > 50 ? (
                  <Image
                    src="/images/yellowflag.png"
                    alt="Yellow Flag"
                    width={22}
                    height={22}
                  /> /* Display yellow flag for chanceOfPlaying > 50 */
                ) : player.chanceOfPlaying &&
                  parseInt(player.chanceOfPlaying) <= 50 ? (
                  <Image
                    src="/images/orangeflag.png"
                    alt="Orange Flag"
                    width={22}
                    height={22}
                  /> /* Display orange flag for chanceOfPlaying <= 50 */
                ) : null}
              </td>
              <td>
                <Image
              ...
              <td className="news-info" style={{ textAlign: "left" }}>
                {player.news}
              </td>
            </tr>
          ))}
