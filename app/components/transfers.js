...
export default async function Transfers() {
  ...
  const dataSortedByTransfersIn = [...data].sort(
    (a, b) => b.transfers_in - a.transfers_in
  );

  // create an array sorted by transfers_out
  const dataSortedByTransfersOut = [...data].sort(
    (a, b) => b.transfers_out - a.transfers_out
  );
  ...
  return (
    <DisplayTransfers
      topPlayersIn={topPlayersIn}
      topPlayersOut={topPlayersOut}
    />
  );
}
