...
async function getFixtures() {
  ...
  const currentGameweek =
    events?.find((event) => event.is_current === true)?.id ??
    events?.find((event) => event.is_next === true)?.id ??
  ...
  try {
    ...
    for (let fixture of Object.values(fixtures)) {
      ...
      const bonusStats = fixture.stats.filter(
        (stat) => stat.identifier === "bps"
      );
      const bonusPoints = fixture.stats.filter(
        (stat) => stat.identifier === "bonus"
      );
      const scorers = fixture.stats.filter(
        (stat) => stat.identifier === "goals_scored"
      );
      const assists = fixture.stats.filter(
