...
export async function POST(req: Request) {
  ...
  const { startPage, endPage, leagueId } = body;
  try {
    console.time(`${startPage}-${endPage}`);

    const data = [];
    // console.log("STARTING LOOP - ", startPage, endPage);
    const countMapTopSetAndForget = new Map();
        ...
        new Promise(async (resolve, reject) => {
          ...
          for (const playerEntry of standingsData?.results) {
            ...
            const picksResponse = await fetch(
              `https://fantasy.premierleague.com/api/entry/${playerEntryId}/transfers/`,
              {
                next: {
                  revalidate: 0,
                },
            ...
            countMapTopSetAndForget.set(playerEntryId, {
                ...
                totalPoints: playerEntry.total,
                transfers: playerPicks.length,
            });   
    ...
    const setAndForget = Array.from(countMapTopSetAndForget.entries()).map(
      ([playerEntryId, { playerName, entryName, currentRank, lastRank, eventTotal, totalPoints, transfers}]) => {
        return {
            playerEntryId,
            entryName,
            playerName,
    ...
    await prisma.topSetAndForget.createMany({
      data: [
        ...setAndForget.map((item) => {
          return {
            ...item,
          };
        }),
